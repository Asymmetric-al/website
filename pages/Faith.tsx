...
const FaithHero = memo(() => (
    <Section className="relative z-10 border-b border-border pb-16 md:pb-24 bg-background">
        <Container>
            <Reveal>
                <div className="max-w-4xl">
                    ...
                    <div className="border-l-2 border-border pl-8">
                        <p className="text-xl text-muted-foreground font-light leading-relaxed max-w-2xl text-balance">
...
const FaithPointItem = memo(({ point, index }: { readonly point: FaithPoint; readonly index: number }) => (
    <Reveal delay={index * 50} className="w-full">
        <div className="group relative grid grid-cols-1 md:grid-cols-12 gap-6 md:gap-12 py-12 border-b border-border transition-colors duration-500 hover:bg-card">
             ...
             <div className="md:col-span-3 flex flex-row md:flex-col items-baseline md:items-start gap-4 md:gap-2 select-none">
                 <span className="text-4xl md:text-5xl font-display font-bold text-muted-foreground/10 group-hover:text-foreground/30 transition-colors duration-500">
             ...
             <div className="md:col-span-9">
                 <p className="text-lg md:text-xl text-muted-foreground font-light leading-relaxed text-balance group-hover:text-foreground/90 transition-colors duration-500">
                     {point.text}
                 </p>
             </div>
        </div>
...
const FaithFooter = memo(() => (
    <Section className="py-24 relative z-10">
        <Container>
            <Reveal delay={200}>
                <div className="flex flex-col items-center justify-center opacity-40 hover:opacity-100 transition-opacity duration-500 text-foreground">
                        <div className="w-16 h-16 border border-border rounded-full flex items-center justify-center mb-6 bg-secondary/50">
                            <span className="text-2xl font-serif italic">†</span>
                        </div>
                        <div className="text-[10px] font-mono uppercase tracking-[0.2em]">Soli Deo Gloria</div>
                </div>
            </Reveal>
        </Container>
    </Section>
...
const Faith: React.FC = () => {
  return (
    <div className="pt-24 min-h-screen bg-background text-foreground font-sans selection:bg-foreground selection:text-background overflow-x-hidden">
        ...
        <Section className="relative z-10 bg-card !pt-0">
